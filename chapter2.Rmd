...
```{r echo = FALSE}
selectInput("data", "Choose a residual plot:",
 c("Residuals vs Fitted values plot" = 1, "Normal QQ-plot" = 2, "Residuals vs Leverage plot" = 5))
```
See a plot:
```{r echo = FALSE}
renderPlot({
  if(input[['data']] == 1){
 plot(regression_trial2, 1, main = "Figure 2 (a). Residuals vs Fitted values plot.")}
    if(input[['data']] == 2){
 plot(regression_trial2, 2, main = "Figure 2 (b). Normal QQ-plot.")}
    if(input[['data']] == 5){
 plot(regression_trial2, 5, main = "Figure 2 (c). Residuals vs Leverage plot.")}
})
```
